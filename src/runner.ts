...
if (!args.length) args.unshift('test/**/*.js');
run(args).then(
...
async function run(patterns: string[]) {
	const files = [];
	for (const items of await Promise.all(patterns.map((pattern) => findFiles(pattern)))) files.push(...items);
	if (!files.length) throw new Error(`no files found for ${patterns.join(' ')}`);
