...
const handlers = {
	'--tap': {
		...
		out(line) {
			...
			this.indent += 1;
			this.out('---');
			this.out(`message: ${failure.message ?? ''}`);
		...
		exit(idx, label, result, children) {
			this.indent -= 1;
			if (!idx) {
				if (result.failure) this.except(result.failure);
			...
			switch (result.status) {
				...
				case '-':
					...
					this.except(result.failure ?? new Error('test failed'));
					break;
				case '!':
...
async function main(mode, patterns) {
	...
	process.exit((process.exitCode = pass ? 0 : 1));
}
